...
def setup_geometry(img_shape, voxel_size, DSD, DSO, shift_pixels, total_angle, shift_sign, downsample_factor=1):
    ...
    # This is the fundamental limit of our detection system
    REAL_DETECTOR_PIXEL_SIZE = 0.050  # mm (50 μm)
    # If your detector has different specifications, change this value
    ...
    # ========== VOXEL-SIZE-FIRST CALCULATION ==========
    # REVERSE CALCULATION: Start from desired voxel size, calculate needed pixel size
    # This is the KEY difference from the original approach
    
    magnification = DSD / DSO
    
    # CRITICAL: Account for downsampling in pixel size calculations
    # When downsampling > 1, each "pixel" in the downsampled projection represents
    # multiple physical detector pixels averaged together
    # Example: downsample=4 means 4×4 = 16 physical pixels → 1 effective pixel
    # Therefore: effective_pixel_size = 50 μm × 4 = 200 μm
    effective_detector_pixel = REAL_DETECTOR_PIXEL_SIZE * downsample_factor
    
    # Required pixel size to achieve the desired voxel size
    # Formula: pixel_size = voxel_size × magnification
    # (This is the inverse of: voxel_size = pixel_size / magnification)
    required_pixel_size = voxel_size_mm * magnification
    
    # Nyquist limit: smallest voxel size achievable with current detector resolution
    # MUST use effective_detector_pixel (which includes downsampling effect)
    # Without downsampling (f=1): Nyquist = 50 μm / magnification
    # With downsampling (f=4): Nyquist = 200 μm / magnification
    # This represents the BEST resolution achievable given current settings
    Nyquist_limit = effective_detector_pixel / magnification
    
    # ========== IMPORTANT: Downsampling affects achievable resolution ==========
    # When downsampling > 1, effective detector pixel becomes larger:
    #   effective_pixel_size = REAL_DETECTOR_PIXEL_SIZE × downsample_factor
    #
    # This REDUCES maximum achievable resolution (Nyquist limit increases)
    # Trade-off: Worse resolution BUT faster reconstruction and smaller files
    # 
    # The Nyquist calculation below accounts for downsampling automatically
    # because required_pixel_size passed here is already downsampled if applicable
    # (downsampling happens in the main script before calling this function)
