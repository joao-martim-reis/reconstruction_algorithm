...
"""
    export_volumes_ASTRA.py
    =======================
    ASTRA-specific version of export_volumes.py.
    
    Identical logic — but TIGRE is NOT imported, so this works in the
    ct_recon conda environment without TIGRE installed.
    
    The original export_volumes.py is kept intact for TIGRE-based scripts.
"""
...
def export_volume_to_nii(volume, geo, source_folder, base_output=None):
    ...
    with open(metadata_file, 'w') as f:
        ...
        f.write(f"  Original shape: {volume.shape} (Z, Y, X)\n")
