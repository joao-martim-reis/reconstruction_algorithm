...
def setup_geometry(img_shape, voxel_size, DSD, DSO, shift_pixels, total_angle,
    ...
    REAL_DETECTOR_PIXEL_SIZE = 0.050  # mm (50 μm hardware pixel)
    ...
    geo.dDetector  = np.array([effective_detector_pixel, effective_detector_pixel])
    ...
    # ── Detector offset (COR calibration) ────────────────────────────────────
    shift_mm      = shift_pixels * effective_detector_pixel
    ...
    if crop_params is not None:
        ...
        crop_shift_px   = new_center - original_center
        crop_shift_mm   = crop_shift_px * effective_detector_pixel * shift_sign
    ...
    geo.rotDetector = np.array([0.0, 0.0, float(detector_tilt)])
