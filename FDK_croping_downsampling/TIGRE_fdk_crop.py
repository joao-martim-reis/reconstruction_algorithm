...
def normalize_projections(projections_raw, I0_override=None):
    print("--> [5/6] Normalizing projections...")
...
def setup_geometry(img_shape, pixel_size, DSD, DSO, shift_pixels, total_angle, shift_sign, voxel_ratio=1.0, crop_offset_px=0.0):
    """
    Setup TIGRE geometry with optional ROI crop offset correction.
    ...
    Args:
        ...
        crop_offset_px: Horizontal offset from ROI cropping (pixels) - for symmetric crop this is 0
    """
    
    print(f"--> Setting up geometry with shift: {shift_pixels:.2f} px, crop offset: {crop_offset_px:.2f} px")
    ...
    # APPLYING SHIFT (with optional crop offset correction)
    # For symmetric ROI crop, crop_offset_px should be ~0
    effective_shift = shift_pixels + crop_offset_px
    shift_mm = effective_shift * pixel_size
...
def main(tiff_folder, configurations, output_folder=None):
    """
    Main reconstruction pipeline with ROI cropping support.
    
    Workflow:
    1. Load images
    2. (Optional) Select and apply symmetric ROI crop to reduce memory
    3. Downsample
    4. Normalize
    5. Reconstruct with FDK
    """
    # 1. Load
    projections = load_images(tiff_folder)
    original_pixel_size = configurations['pixel_size']
    
    # 2. ROI Cropping (optional - enable with 'enable_roi_crop': True)
    crop_info = None
    if configurations.get('enable_roi_crop', False):
        roi_bounds = select_symmetric_roi_crop(projections)
        if roi_bounds is not None:
            projections, crop_info = apply_roi_crop(projections, roi_bounds)
            print(f"    ✓ ROI cropping applied - memory saved!")
        else:
            print("    ⚠ ROI cropping cancelled - using full detector")
    else:
        print("--> ROI cropping disabled (set 'enable_roi_crop': True to enable)")
    ...
    if configurations['downsample'] > 1:
        ...
        pixel_size = original_pixel_size * f  # effective pixel size increases by f
    else:
        pixel_size = original_pixel_size

    # 4. Get calibrated shift (adjusted for downsampling and crop)
    ...
    # For symmetric crop, horizontal offset should be ~0, so no additional correction needed
    crop_offset_px = 0.0  # Symmetric crop maintains center alignment
    ...
    if crop_info:
        print(f"    Crop offset: {crop_info['horizontal_offset_px']:.2f} px (should be ~0 for symmetric crop)")

    # 5. Select I0 ROI for normalization
    roi_background = selecionar_roi_I0(generate_collapsed_sinogram(projections))
    mean_I0 = get_I0_from_roi(generate_collapsed_sinogram(projections), roi_background)

    # 6. Normalize projections
    projections_norm = normalize_projections(projections, I0_override=mean_I0)
    
    del projections
    gc.collect()

    # 7. Geometry and Reconstruction
...
if __name__ == "__main__":
    CONFIG = {
        ...
        # ROI CROPPING (NEW!)
        'enable_roi_crop': True,  # Set True to enable interactive ROI selection
        # Interactive workflow:
        # 1. Click pixel for horizontal center
        # 2. Adjust crop width with slider
        # 3. Click top and bottom lines for vertical crop
        # → Symmetric horizontal crop maintains center alignment (no geometry offset issues)
