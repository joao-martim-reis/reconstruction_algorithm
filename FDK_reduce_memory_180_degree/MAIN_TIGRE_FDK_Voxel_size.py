...
def main(tiff_folder, configurations, output_folder=None):
    ...
    projection_fraction = configurations.get('projection_fraction', 1.0)
    projections_raw = load_images(tiff_folder, projection_fraction=projection_fraction)  # Step 1.1: Load raw TIFF projection images
    sino_raw = generate_collapsed_sinogram(projections_raw)  # Step 1.2: Generate collapsed sinogram for I0 reference selection
    angle_span_deg = float(np.rad2deg(configurations['total_angle']))
    roi_background = selecionar_roi_I0(sino_raw, angle_span_deg=angle_span_deg)  # Step 1.3: User selects background ROI and calculates mean I0 value
    ...
    geo, angles = setup_geometry(  # Step 3.3: Setup TIGRE geometry with ADJUSTED voxel size
        ...
        detector_tilt=configurations.get('detector_tilt', 0)  # ← Detector in-plane tilt (rad)
    )
    ...
    # RECONSTRUCTION
    volume = algs.fdk(input_data, geo, angles, filter=configurations['filter_type'])  # Step 3.5: EXECUTE FDK reconstruction algorithm
...
if __name__ == "__main__":
    ...
    CONFIG = {
        ...
        'shift_sign': 1,
        'total_angle': np.pi,  # 180° half-scan mode
        'projection_fraction': 0.5,  # Use only first 50% of TIFF projections
